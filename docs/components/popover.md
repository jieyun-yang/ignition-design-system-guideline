...
title: Popover
category: Components
url: https://design.ignitionapp.com/25ddf264c/p/2978051
scraped: 2025-11-16T03:25:23.282Z
---

# Popover
This documentation outlines the guidelines for using the Popover component within the design system.
...
*   **NPE (standard)**:
    *   **Trigger**: User clicks a Call To Action (CTA).
    *   **Position**: Popover displays either above or below the CTA.
    *   **Closing**: Users **cannot** click outside to close the Popover, as it requires an explicit save action (e.g., via a "Save" button) to commit changes.
    *   **Interaction**: Does not block underlying interactions.
...
No specific accessibility considerations were mentioned in the provided content. However, general accessibility principles such as keyboard navigation, proper focus management, and ARIA attributes should be applied to Popovers to ensure they are usable by all users. The "Doesn't block interactions" guideline implicitly contributes to a better user experience, including for assistive technologies.

## Code Examples

No code snippets or examples were provided in the extracted content.

## Related Components

*   **Drawer**: If content for a Popover becomes too long or complex, it should be placed in a [Drawer](/components/drawer) component instead.