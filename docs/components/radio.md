...
title: Radio
category: Components
url: https://design.ignitionapp.com/25ddf264c/p/2978061
scraped: 2025-11-16T03:10:46.094Z
---

# Radio
This documentation outlines the guidelines for using Radio components within the design system, ensuring consistency, usability, and accessibility across products.
...
*   **Example (Client Payments)**: Displayed as a vertical list when more than 3 options are possible.
*   **Example (Search Filters)**: Displayed as a vertical list within a small popover due to space constraints and more than 3 options. Secondary input for a selection is only displayed once the radio is selected.
...
*   **Example (Proposal Start Date)**: Used as a radio button group within a larger container for only 2 options. More inputs are displayed upon selection.
...
*   **Selected**: The state indicating that an option has been chosen.
    *   *Behavior*: A selected background may be displayed if the option is selected.
...
*   **Default Selection**: Always have one option pre-selected by default for radio and radio button groups to provide a clear starting point.
    *   *Example*: A selection is always selected by default for radio and radio button groups in NPE and filter selections.
